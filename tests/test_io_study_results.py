...
def test_load_study_manager_export_returns_wide_multiindex_columns(tmp_path):
    result = cw.io.load_study_manager_export(_write_csv(tmp_path))
...
def test_load_study_manager_export_preserves_day_level_mismatch_summary(tmp_path):
    result = cw.io.load_study_manager_export(_write_csv(tmp_path))
...
def test_load_study_manager_export_combines_date_and_times(tmp_path):
    result = cw.io.load_study_manager_export(_write_csv(tmp_path))
...
def test_empty_sample_values_remain_unknown(tmp_path):
    ...
    result = cw.io.load_study_manager_export(path)
    samples = cw.logs.extract_sample_events_from_summary(result)
...
def test_load_study_manager_export_supports_multiple_days_and_ignores_google_fit(
    tmp_path,
):
...
"""
    result = cw.io.load_study_manager_export(_write_csv(tmp_path, content))
    awakening = cw.logs.extract_awakening_events_from_summary(result)
...
def test_load_study_manager_export_does_not_require_study_name(tmp_path):
...
def test_load_study_manager_export_sorts_samples_naturally(tmp_path):
...
def test_load_study_manager_export_rejects_invalid_schema(tmp_path, content):
    with pytest.raises(SchemaError):
        cw.io.load_study_manager_export(_write_csv(tmp_path, content))


def test_load_study_manager_export_rejects_duplicate_participants(tmp_path):
